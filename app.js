...
const updatedTodoTable = (dbObject) => {
  return {
    id: dbObject.id,
    todo: dbObject.todo,
    ...
    case hasPriorityAndStatusProperties(request.query):
      if (priority === "HIGH" || priority === "MEDIUM" || priority === "LOW") {
        if (
          ...
          dbUser = await db.all(getTodosQuery);
          response.send(dbUser.map((eachTodo) => updatedTodoTable(eachTodo)));
        } else {
          response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(updatedTodoTable(todoQuery));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    response.send(dateQuery.map((eachDate) => updatedTodoTable(eachDate)));
  } else {
    response.status(400);
